...
abaloneAgeRecipe <-
  ...
  step_dummy_multi_choice(starts_with("type")) %>% # creates dummy variables for type
...
```{r}
predictThis <- data.frame(type="F", longest_shell=0.50, diameter=0.10, height=0.30, whole_weight=4, shucked_weight=1, viscera_weight=2, shell_weight=1, rings=0, age=0) %>% 
  print()
```
...
abaloneTrainRes <- predict(lmFit, new_data = predictThis %>% select(-rings))
abaloneTrainRes %>%
  head()
